...
function onYouTubeIframeAPIReady() {
	player = new YT.Player('youtube-player', {
		height: '0',
		width: '0',
		videoId: nasheeds[currentNasheedIndex].videoId,
		playerVars: {
			'playsinline': 1,
			'enablejsapi': 1,
			'autoplay': 0,
		...
		events: {
			'onReady': function (event) {
				...
				document.getElementById('prev').addEventListener('click', prevNasheed);
				document.getElementById('next').addEventListener('click', nextNasheed);
			...
			'onStateChange': function (event) {
				...
				if (event.data == YT.PlayerState.PLAYING) {
					...
					status.textContent = "Now playing nasheed";
					...
					status.textContent = "Nasheed paused";
					clearInterval(updateInterval);
				}
					...
					status.textContent = "Buffering nasheed...";
				}
			}
		}
	});
...
function loadNasheed(nasheedIndex) {
	const nasheed = nasheeds[nasheedIndex];

	document.getElementById('nasheed-title').textContent = nasheed.title;
	document.getElementById('nasheed-artist').textContent = nasheed.artist;
	document.querySelector('.album-art img').src = nasheed.cover;
	document.getElementById('nasheed-duration').textContent = nasheed.duration;

	if (player) {
		player.loadVideoById(nasheed.videoId);
		player.pauseVideo();
		isPlaying = false;
...
function prevNasheed() {
	currentNasheedIndex--;
	if (currentNasheedIndex < 0) {
		currentNasheedIndex = nasheeds.length - 1;
	}
	loadNasheed(currentNasheedIndex);
	if (isPlaying) {
		player.playVideo();
...
function nextNasheed() {
	if (isShuffle) {
		let newIndex;
		do {
			newIndex = Math.floor(Math.random() * nasheeds.length);
		} while (newIndex === currentNasheedIndex);
		currentNasheedIndex = newIndex;
	}
	else {
		currentNasheedIndex++;
		if (currentNasheedIndex >= nasheeds.length) {
...
function toggleRepeat() {
	...
	if (isRepeat) {
		...
		document.getElementById('status').textContent = "Repeat mode enabled";
	}
	else {
		repeatBtn.classList.remove('active');
		repeatBtn.innerHTML = '<i class="fas fa-redo"></i> Repeat';
		document.getElementById('status').textContent = "Repeat mode disabled";
	}
}
